...
const privacyContent: Record<LanguageCode, LegalContent> = {
  'zh-CN': {
    ...
    sections: [
      ...
      { title: 'Apple Health 及健康数据', body: '我们通过 Apple HealthKit（iOS）和 Google Fit（Android）读取您的运动数据（步数、消耗热量）。\n\n我们郑重承诺：\n\n• HealthKit 数据仅用于健康与健身目的，即在 Lock 应用内向您展示每日运动进度\n• 不会将 HealthKit 数据用于任何广告目的\n• 不会将 HealthKit 数据出售给第三方或数据经纪商\n• 不会将 HealthKit 数据与广告平台共享\n• 所有健康数据访问均需您主动授权，您可随时在设备的「设置 → 隐私 → 健康」中撤回授权\n\n如您撤回授权，Lock 将停止读取相关数据，但已记录的历史数据不会被自动删除，您可在账户设置中手动删除所有数据。' },
  ...
  'en-US': {
    ...
    sections: [
      ...
      { title: 'Apple Health & Health Data', body: 'We access your activity data (steps, active calories) through Apple HealthKit (iOS) and Google Fit (Android).\n\nWe commit to the following:\n\n• HealthKit data is used exclusively for health and fitness purposes — to display your daily activity in Lock\n• HealthKit data will never be used for advertising or marketing\n• HealthKit data will never be sold to third parties or data brokers\n• HealthKit data will never be shared with advertising platforms\n• All health data access requires your explicit permission; you can revoke it at any time in Settings → Privacy → Health\n\nIf you revoke access, Lock will stop reading new health data. Existing synced data will remain until you delete your account or clear your data manually.' },
  ...
  'zh-TW': {
    ...
    sections: [
      ...
      { title: 'Apple Health 及健康數據', body: '我們透過 Apple HealthKit（iOS）和 Google Fit（Android）讀取您的運動數據（步數、消耗熱量）。\n\n我們鄭重承諾：\n\n• HealthKit 數據僅用於健康與健身目的，即在 Lock 應用內向您展示每日運動進度\n• 不會將 HealthKit 數據用於任何廣告目的\n• 不會將 HealthKit 數據出售給第三方或數據經紀商\n• 不會將 HealthKit 數據與廣告平台共享\n• 所有健康數據訪問均需您主動授權，您可隨時在「設置 → 隱私 → 健康」中撤回授權' },
  ...
  'ja-JP': {
    ...
    sections: [
      ...
      { title: 'Apple Healthと健康データ', body: 'Apple HealthKit（iOS）とGoogle Fit（Android）を通じて運動データ（歩数、活動カロリー）にアクセスします。\n\n当社の約束：\n\n• HealthKitデータは健康・フィットネス目的のみに使用されます（Lockアプリ内での日々の活動表示）\n• HealthKitデータを広告目的に使用することは一切ありません\n• HealthKitデータを第三者やデータブローカーに販売することはありません\n• HealthKitデータを広告プラットフォームと共有することはありません\n• すべての健康データアクセスには明示的な許可が必要です。「設定 → プライバシー → ヘルスケア」からいつでも許可を取り消すことができます' },
  ...
  'ko-KR': {
    ...
    sections: [
      ...
      { title: 'Apple Health 및 건강 데이터', body: 'Apple HealthKit(iOS)와 Google Fit(Android)를 통해 운동 데이터(걸음수, 활동 칼로리)에 접근합니다.\n\n당사의 약속:\n\n• HealthKit 데이터는 건강 및 피트니스 목적으로만 사용됩니다 (Lock 앱 내 일일 활동 표시)\n• HealthKit 데이터를 광고 목적으로 사용하지 않습니다\n• HealthKit 데이터를 제3자나 데이터 브로커에 판매하지 않습니다\n• HealthKit 데이터를 광고 플랫폼과 공유하지 않습니다\n• 모든 건강 데이터 접근에는 명시적 허가가 필요합니다. 설정 → 개인정보 → 건강에서 언제든지 권한을 철회할 수 있습니다' },
